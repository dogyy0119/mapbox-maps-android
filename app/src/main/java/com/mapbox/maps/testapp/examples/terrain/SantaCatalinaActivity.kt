...
class SantaCatalinaActivity : AppCompatActivity() {
    ...
    mapboxMap.loadStyle(
      style(styleUri = Style.SATELLITE_STREETS) {
        ...
        +image(FOREGROUND_ICON) {
          bitmap(BitmapFactory.decodeResource(resources, R.drawable.mapbox_mylocation_icon_default))
        }
      ...
      // wait for map to become idle before making request
      mapboxMap.addOnMapChangedListener {
        if (it == MapChange.DID_BECOME_IDLE) {
  ...
  private fun animateRoute(lineString: LineString) {
    ...
    val locationLayer = mapboxMap.getStyle()!!.getLayerAs<LocationIndicatorLayer>(
      LOCATION_LAYER_ID
    )
    ...
    // get a reference to elevations of the animation
    // note: to call getElevation the coordinate needs to be in the viewport
    val startElevation = mapboxMap.getElevation(POINT_START)!!.toDouble()
    val endElevation = mapboxMap.getElevation(POINT_END)!!.toDouble()
    val minElevation = minOf(startElevation, endElevation)
    val maxElevation = maxOf(startElevation, endElevation)
    ...
    timeAnimator.setTimeListener { animator, totalTime, _ ->
      ...
      val elevation = minOf(maxElevation, (maxElevation * phase) + minElevation)
      ...
      // calculate an elevation difference between the user position and the camera
